...
const Criterion = () => {
  ...
  const selectedArray = [
    {
      headerText: "Everyone on verxio",
      description: "All verxio earners can participate",
      value: "all",
    },
    {
      headerText: "Point Specific",
      description: "Those with a specific point balance can participate",
      value: "point",
    },
    {
      headerText: "New Earners",
      description: "Those that have never participated in any quest before",
    },
  ];
  return (
      ...
      {showOptions && (
        <section className="absolute bg-white z-[50] w-[600px] top-[8rem] right-[2rem] flex flex-col items-center gap-3 rounded-lg p-3 shadow-sm">
          <select
            ...
            className="w-full bg-white border border-primary rounded-lg p-2 focus:outline-none"
          >
            {selectedArray.map((item, index) => (
              <option key={index} value={item.value}>
                <div className="w-full bg-white border-2 border-primary rounded-lg p-2 flex flex-col items-start cursor-pointer transition-all duration-300 hover:shadow-sm">
                  <h2 className="semibold text-[18px]">{item.headerText}</h2>
                  <p className="normal text-[14px]">{item.description}</p>
                </div>
              </option>
            ))}
          </select>
